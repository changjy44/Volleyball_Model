# in pf1, we half the team's blocking
...
block_dig_cols = ['block_points', 'block_errors', 'block_touches', 'dig_sucess', 'dig_error']
reception_cols = ['reception_perfect', 'reception_errors', 'reception_attempts']
...
def get_players(team, other_aggregated_stats, train_matches, all_teams_processed_model, year, other_team):
    ...
    all_other_matches = [(match['away_players'], match['num_sets']) for match in train_matches[team]['matches'] if match['other_team'] == other_team]
    
    if len(all_other_matches) == 0:
        block_dig_other_specific = block_dig_other
    else:
        all_other_matches_normalised = [normalise_block_dig(players, num_sets) for players, num_sets in all_other_matches]
        block_dig_other_specific = [mean(stats) for stats in zip(*all_other_matches_normalised)]
    
    # Then, average the stats
    preaverage = [block_dig_other, block_dig_other_specific]
    averaged_block_dig = [mean(stats) for stats in zip(*preaverage)]
    
    teamA_attack = model.predict_attack(averaged_block_dig)[0]
...
def generate_pat(year, train_matches, predict_matches, all_teams_processed_model, league):
    ...
    for match in predict_matches:
        ...
        teamA_players = get_players(teamA, teamB_aggregated_stats, train_matches, all_teams_processed_model, year, teamB)
        teamB_players = get_players(teamB, teamA_aggregated_stats, train_matches, all_teams_processed_model, year, teamA)
...
class PF1:
    ...
    def model_get_players(self, teamA, teamB, matches_processed, ml_models, year):
        ...
        teamA_players = get_players(teamA, teamB_aggregated_stats, matches_processed, ml_models, year, teamB)
        teamB_players = get_players(teamB, teamA_aggregated_stats, matches_processed, ml_models, year, teamA)
    ...
    def generate_ml_stats(self, ml_processed, ml_models, team, other_team, match_id, year, league):
        ...
        all_other_matches = [(match['away_players'], match['num_sets']) for match in ml_processed[team]['matches'] if match['other_team'] == other_team]
        ...
        team_attack = model.predict_attack(averaged_block_dig)[0]
